...
export default {
    config: {
        ...
        usage: [ "{pn}صور <كلمة البحث> <العدد> — مثال: صور cars 12", '{pn}صور "<كلمة البحث>" <العدد> — مثال: صور "sunset" 10', "{pn}صور <كلمة البحث> — بدون تحديد عدد (افتراضي 5 صور)" ]
    },
    onStart: async function({api: api, event: event, args: args}) {
        const {threadID: threadID, messageID: messageID} = event;
        const {query: query, count: count} = parseArgs(args);
        if (!query) {
        ...
        try {
            ...
            return api.sendMessage(`😕 لم أجد صوراً لكلمة "${query}".\n${error.message.substring(0, 150)}`, threadID, null, messageID);
        } finally {
