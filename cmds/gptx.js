...
const TEXT_ENDPOINTS = [ {
    name: "overchat-claude",
    base: BASE_CEDDS,
    path: "/ai/overchat-claude",
    bodyKey: "message"
}, {
    name: "overchat-deepseek",
    base: BASE_CEDDS,
    path: "/ai/overchat-deepseek",
    bodyKey: "message"
}, {
    name: "overchat-qwen",
    base: BASE_CEDDS,
    path: "/ai/overchat-qwen",
    bodyKey: "message"
}, {
    name: "overchat",
    base: BASE_CEDDS,
...
function cleanReply(text) {
    if (!text) return text;
    return text.replace(/@DONE@/gi, "").replace(/@\w+@/g, "").replace(/(-=\s*|\s*=-)+/g, " ").replace(/\*{2,}/g, "").replace(/_{2,}/g, "").replace(/#{1,6}\s*/g, "").replace(/[-=]{3,}/g, "").replace(/[ \t]{2,}/g, " ").replace(/\n{3,}/g, "\n\n").trim();
}
...
function setThreadCache(map, key, value) {
    map.delete(key);
    if (map.size >= MAX_THREAD_CACHE) map.delete(map.keys().next().value);
    map.set(key, value);
    return value;
}
...
async function isActive(threadID) {
    ...
    setThreadCache(_activeMap, threadID, val);
    return val;
}

async function setActive(threadID, value) {
    setThreadCache(_activeMap, threadID, value);
    await saveCtx(COLLECTION_ACTIVE, threadID, {
        active: value
    });
}
...
async function loadHistory(threadID) {
    ...
    setThreadCache(_historyMap, threadID, h);
    return h;
}

async function saveHistory(threadID, history) {
    setThreadCache(_historyMap, threadID, history);
...
async function callImageGen(prompt) {
    ...
    for (const ep of IMAGEGEN_ENDPOINTS) {
        try {
            ...
            const {data: data} = await http.get(`${url}?${params}`, {
                timeout: TIMEOUT_MS,
                headers: {
                    "User-Agent": "Mozilla/5.0"
                }
...
function detectImageUrl(event) {
    ...
    for (const att of sources) {
        ...
        if ([ "photo", "image", "sticker", "animated_image" ].includes(type)) {
            const url = att.largePreviewUrl || att.previewUrl || att.largePreviewUri || att.previewUri || att.url || att.uri || att.thumbnailUrl;
            if (url) return url;
        }
    }
    return null;
...
async function handleMessage(api, event, message, prompt, registerReply) {
    const {threadID: threadID, messageID: messageID, senderID: senderID} = event;
    ...
    if (setMatch) {
        ...
        if (!endpName) {
            const list = Object.keys(MODEL_ALIASES).filter((k => /^[a-z]/.test(k))).join(", ");
            return message.reply(`❓ نموذج غير معروف: "${alias}"\n\nالنماذج المتاحة:\n${list}`);
        }
        setThreadCache(_modelMap, threadID, endpName);
        return message.reply(`✅ تم تغيير النموذج إلى: ${MODEL_DISPLAY[endpName]}\n` + `gptx set claude  — للعودة لكلود\n` + `gptx set reset   — للترتيب الافتراضي (أفضل متاح)`);
    }
    ...
    if (lowerPrompt === "models" || lowerPrompt === "نماذج") {
        ...
        const lines = Object.entries(MODEL_DISPLAY).map((([k, v]) => `${current === k ? "✅" : "•"} ${v}  →  gptx set ${TEXT_ENDPOINTS.find((e => e.name === k))?.name?.replace("overchat-", "") ?? k}`));
    ...
    if (CMD_ON.includes(lowerPrompt)) {
        ...
        return message.reply("✅ تم تفعيل الوضع التلقائي!\n" + "البوت سيرد على كل رسالة في الغروب.\n" + "لإيقافه: gptx off");
    }
    ...
    const imgMatch = lowerPrompt.match(/^img\s+(.+)$/s) || prompt.match(/^img\s+(.+)$/is);
    if (imgMatch) {
        ...
        try {
            ...
            return api.sendMessage({
                body: `🎨 ${imgPrompt}`,
                attachment: await http.get(imgUrl, {
                    responseType: "stream"
                }).then((r => r.data))
            }, threadID, messageID);
        } catch (e) {
    ...
    const detectMatch = prompt.match(/^detect\s+(.+)$/is) || prompt.match(/^كشف\s+(.+)$/s);
    if (detectMatch) {
        ...
        try {
            ...
            return message.reply(`🔍 نتيجة الكشف:\n\n` + `👤 بشري: ${human}%\n` + `🤖 AI: ${ai}%\n\n` + `📋 ${verdict}`);
        } catch (e) {
    ...
    const finalPrompt = prompt ? buildPromptWithHistory(history, prompt) : imageOnlyPrompt;
    ...
    try {
        if (imageUrl) {
            ...
            const imgFallbackMsg = prompt ? `${finalPrompt}\n[رابط الصورة: ${effectiveUrl}]` : `${imageOnlyPrompt}\n[رابط الصورة: ${effectiveUrl}]`;
            try {
    ...
    if (registerReply) {
        registerReply(info.messageID, {
            threadID: threadID
        }, (async ({api: api, event: event, message: message}) => {
...
export default {
    config: {
        ...
        usage: [ "{pn}gptx <سؤالك> — بدء محادثة", "{pn}gptx on/off — تفعيل/إيقاف الرد التلقائي", "{pn}gptx set <نموذج> — تغيير النموذج (claude/deepseek/qwen/gpt/goody/opera...)", "{pn}gptx models — عرض النماذج المتاحة", "{pn}gptx img <وصف> — توليد صورة بالذكاء الاصطناعي", "{pn}gptx detect <نص> — كشف هل النص من AI أم لا", "رد على صورة + gptx — تحليل الصورة", "{pn}gptx مسح — مسح ذاكرة المحادثة" ]
    },
    ...
    onChat: async ({api: api, event: event, message: message}) => {
        const {body: body, threadID: threadID} = event;
