...
const HEADERS = {
    "User-Agent": "SunkenBot/2.0 (manga command)"
};
...
function cleanQuery(raw) {
    return raw.replace(/["'`ʼ’]/g, "").replace(/[^\p{L}\p{N}\s]/gu, " ").replace(/\s+/g, " ").trim().toLowerCase();
}
...
function bestTitle(manga) {
    const attrs = manga.attributes || {};
    return attrs.title?.en || attrs.title?.ja || Object.values(attrs.title || {})[0] || "بدون عنوان";
}
...
const CHAPTER_QUERY_TTL = 5 * 60 * 1e3;
...
function pickBestChapterResult(rawResults, targetNum) {
    const valid = (rawResults || []).filter((item => item?.attributes && isExactChapterMatch(item.attributes.chapter, targetNum) && isReadableChapter(item.attributes)));
    ...
    return {
        id: best.id,
        lang: best.attributes.translatedLanguage
    };
}
...
async function findBestChapterForLanguage(mangaId, chapterCandidates, lang, targetNum) {
    const settled = await Promise.allSettled(chapterCandidates.map((candidate => queryChaptersRaw(mangaId, candidate, lang))));
    const combined = [];
...
async function findNearestChapterAsLastResort(mangaId, targetNum) {
    try {
        ...
        return {
            id: nearest.id,
            lang: nearest.attributes.translatedLanguage
        };
    } catch (_) {
        return null;
    }
...
const downloadAllWithLimit = (pageUrls, limit = DOWNLOAD_CONCURRENCY) => downloadWithLimit(pageUrls, downloadImage, limit);
...
function formatRuns(runs) {
    const shown = runs.slice(0, MAX_RUNS_SHOWN).map((r => r.from === r.to ? `الفصل ${r.from} موجود` : `من ${r.from} الى ${r.to} موجودة`));
    let text = shown.join("\n");
...
async function runChapterRangeSearch({api: api, threadID: threadID, messageID: messageID, rawName: rawName, mangaQuery: mangaQuery}) {
    let statusMsgId = null;
    try {
        statusMsgId = await new Promise((resolve => {
            global.safeSend(api, `⏳ جاري البحث عن المانجا...\n📖 ${rawName}`, threadID, ((err, info) => resolve(err ? null : info?.messageID || null)), messageID);
        }));
    } catch (_) {}
    const updateStatus = async text => {
        try {
            if (statusMsgId) await api.editMessage(text, statusMsgId); else global.safeSend(api, text, threadID, null, messageID);
        } catch (_) {
    ...
    try {
        ...
        try {
            ...
            throw {
                userMsg: "❌ تعذر الاتصال بخادم المانجا.\nحاول لاحقاً."
            };
        }
        if (!candidates.length) {
            throw {
                userMsg: "❌ لم يتم العثور على مانجا بهذا الاسم."
            };
        }
        const {manga: manga, score: score} = pickBestManga(mangaQuery, candidates);
        if (!manga || score < MIN_MATCH_SCORE) {
            throw {
                userMsg: "❌ لم أتمكن من العثور على المانجا."
            };
        }
        ...
        if (!numbers.length) {
            throw {
                userMsg: "❌ لا توجد أي فصول مترجمة للعربية لهذه المانجا."
            };
        }
        numbers.sort(((a, b) => a - b));
        const {runs: runs, gaps: gaps} = findRuns(numbers);
        ...
        try {
            if (statusMsgId) await api.editMessage(userMsg, statusMsgId); else global.safeSend(api, userMsg, threadID, null, messageID);
        } catch (_) {
...
const BRIDGE_HEADERS_3ASQ = INTERNAL_TOKEN_3ASQ ? {
    "X-Internal-Token": INTERNAL_TOKEN_3ASQ
} : {};

const POLL_INTERVAL_MS_3ASQ = 2e3;

const POLL_TIMEOUT_MS_3ASQ = 120 * 1e3;

async function createJob3asq(manga, chapter) {
    const {data: data} = await http.post(`${BRIDGE_URL_3ASQ}/manga-bridge/jobs`, {
        source: SOURCE_3ASQ,
        manga: manga,
        chapter: chapter
    }, {
        headers: BRIDGE_HEADERS_3ASQ,
        timeout: 15e3
    });
    return data.job_id;
}
...
async function pollJob3asq(jobId) {
    ...
    while (Date.now() < deadline) {
        const {data: data} = await http.get(`${BRIDGE_URL_3ASQ}/manga-bridge/jobs/${jobId}`, {
            headers: BRIDGE_HEADERS_3ASQ,
            timeout: 15e3
        });
...
const downloadAllWithLimit3asq = (jobId, count, limit = 6) => downloadWithLimit(Array.from({
    length: count
}, ((_, i) => i)), (i => downloadJobImage3asq(jobId, i)), limit);

async function tryFallback3asq({api: api, threadID: threadID, messageID: messageID, rawName: rawName, chapterNumber: chapterNumber}) {
    ...
    try {
        ...
        for (let i = 0; i < validFiles.length; i += MAX_PER_GROUP) {
            ...
            await new Promise(((resolve, reject) => {
                global.safeSend(api, {
                    body: isFirst ? `📖 ${rawName} — الفصل ${chapterNumber} (مصدر بديل)` : "",
...
export default {
    config: {
        ...
        usage: [ "{pn}مانجا <اسم المانجا> <رقم الفصل> — مثال: {pn}مانجا one piece 13", "{pn}مانجا <اسم المانجا> <رقم الفصل> <لغة> — مثال: {pn}مانجا one piece 13 en", "{pn}مانجا <اسم المانجا> <رقم الفصل> ar — مثال: {pn}مانجا attack on titan 5 ar", "{pn}مانجا <اسم المانجا> (بدون رقم فصل) — يعرض نطاقات الفصول العربية المتوفرة/المفقودة، مثال: {pn}مانجا one piece" ]
    },
    onStart: async function({api: api, event: event, args: args}) {
        const {threadID: threadID, messageID: messageID} = event;
        if (!args.length) {
            return global.safeSend(api, "📖 قارئ المانجا\n\n" + "📝 الاستخدام: manga [اسم المانجا] [رقم الفصل]\n\n" + "💡 مثال:\n  manga one piece 13\n  manga one piece 13 en", threadID, null, messageID);
        }
        ...
        if (!rawName) {
            return global.safeSend(api, "📖 قارئ المانجا\n\n" + "📝 الاستخدام: manga [اسم المانجا] [رقم الفصل]\n\n" + "💡 مثال:\n  manga one piece 13", threadID, null, messageID);
        }
        const mangaQuery = cleanQuery(rawName);
        if (!mangaQuery) {
        ...
        try {
            ...
            try {
                ({chapterId: chapterId, lang: lang, availableLangs: availableLangs} = await resolveChapter(mangaId, chapterNumber, requestedLang));
            } catch (err) {
                throw {
                    userMsg: "❌ تعذر الاتصال بخادم المانجا.\nحاول لاحقاً."
                };
            ...
            if (!chapterId) {
                ...
                throw {
                    userMsg: `❌ الفصل ${chapterNumber} غير متوفر بـ${langLabel}.`
                };
            }
            let pageUrls;
            try {
            ...
            if (!validFiles.length) {
                throw {
                    userMsg: "❌ فشل تحميل صفحات الفصل. حاول مرة أخرى."
                };
            }
            let allSent = true;
            ...
            for (let i = 0; i < validFiles.length; i += MAX_PER_GROUP) {
                ...
                const isFirst = i === 0;
                const body = totalGroups > 1 ? `📖 ${mangaTitle} — الفصل ${chapterNumber} (${groupNum}/${totalGroups})` : `📖 ${mangaTitle} — الفصل ${chapterNumber}`;
                try {
                    await new Promise(((resolve, reject) => {
                        global.safeSend(api, {
                            body: body,
            ...
            if (!allSent || validFiles.length !== pageUrls.length) {
                global.safeSend(api, "⚠️ تم إرسال جزء من الفصل فقط.\nيمكنك إعادة المحاولة.", threadID, null, null);
            }
        } catch (err) {
            const userMsg = err?.userMsg || `❌ حدث خطأ غير متوقع: ${err?.message?.substring(0, 80) || ""}`;
            const shouldTryFallback = userMsg.includes("لم يتم العثور على مانجا") || userMsg.includes("غير متوفر بـ") || userMsg.includes("تعذر الاتصال بخادم المانجا");
            if (shouldTryFallback) {
                const sentFallback = await tryFallback3asq({
                    api: api,
                    threadID: threadID,
                    messageID: messageID,
                    rawName: rawName,
