...
export default {
    ...
    onStart: async ({api: api, event: event, args: args, message: message}) => {
        ...
        try {
            ...
            const statusMsg = await new Promise(((res, rej) => global.safeSend(api, "🎨 جاري توليد الصورة...", threadID, ((e, i) => e ? rej(e) : res(i)), messageID)));
            ...
            if (buf.length < 1e3) throw new Error("بيانات الصورة فارغة أو قصيرة جداً");
            ...
            try {
                if (statusMsg?.messageID) await api.unsendMessage(statusMsg.messageID, threadID);
            } catch (_) {}
            ...
            const errMsg = err.code === "ECONNABORTED" ? "⏱ انتهت مهلة التوليد، حاول لاحقاً." : `❌ فشل توليد الصورة: ${err.message?.substring(0, 100)}`;
