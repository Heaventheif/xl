...
const SERIES_CACHE_TTL_MS = 5 * 60 * 1e3;

const LABEL_CACHE_TTL_MS = 30 * 60 * 1e3;
...
async function fetchSeriesList({force: force = false} = {}) {
    const now = Date.now();
...
async function getMangaLabel(mangaUrl) {
    ...
    const {data: data} = await http.post(`${getHfBase()}/comic/label`, {
        series_url: mangaUrl
    }, {
        timeout: 2e4,
        headers: {
    ...
    if (label) {
        if (labelCache.size >= MAX_LABEL_CACHE) labelCache.delete(labelCache.keys().next().value);
        labelCache.set(mangaUrl, {
            fetchedAt: now,
            label: label
        });
    }
...
function matchChapter(chapters, query) {
    ...
    if (candidates.length > 1) {
        ...
        return {
            match: exactByTitle || candidates[0],
            ambiguous: true,
...
async function fetchChapterImages(chapterUrl) {
    const {data: data} = await http.post(`${getHfBase()}/comic/images`, {
...
const downloadAllWithLimit = (urls, refererUrl, limit = DOWNLOAD_CONCURRENCY) => downloadWithLimit(urls, ((url, i) => downloadImage(url, i, refererUrl)), limit);
...
export default {
    config: {
        ...
        usage: [ "{pn}comic <اسم السلسلة> — عرض آخر الفصول المتاحة", "{pn}comic <اسم السلسلة> <رقم الفصل> — إرسال فصل محدد، مثال: {pn}comic moon knight 7.5", "{pn}comic <اسم السلسلة> annual <رقم> — إرسال عدد سنوي محدد" ]
    },
    onStart: async ({api: api, event: event, args: args}) => {
        const {threadID: threadID, messageID: messageID} = event;
        if (args.length === 0) {
            return global.safeSend(api, `⚠️ الاستخدام:\n` + `• .comic <اسم السلسلة> [رقم الفصل]\n` + `• .comic <اسم السلسلة>                ← لعرض آخر الفصول\n\n` + `📌 أمثلة:\n` + `• .comic kill all immortals 1\n` + `• .comic moon knight 7.5\n` + `• .comic absolute batman annual 1`, threadID, null, messageID);
        }
        ...
        try {
            ...
            if (!label) {
                return global.safeSend(api, `❌ تعذّر قراءة معرّف السلسلة من الصفحة:\n${mangaUrl}`, threadID, null, messageID);
            }
            const chapters = await getChaptersForLabel(label);
            if (chapters.length === 0) {
                return global.safeSend(api, `❌ لا توجد فصول منشورة بعد لـ "${mangaTitle}".`, threadID, null, messageID);
            }
            if (!chapterQuery) {
                ...
                return global.safeSend(api, `📖 ${mangaTitle}\n` + `🔖 ${chapters.length} فصل متاح\n\n` + `🆕 آخر الفصول:\n${lines.join("\n")}\n\n` + `📌 لإرسال فصل: .comic ${rawMangaName} <رقم أو annual 1 ...>`, threadID, null, messageID);
            }
            const {match: match, ambiguous: ambiguous, candidates: candidates} = matchChapter(chapters, chapterQuery);
            if (!match) {
                const sample = chapters.slice(0, 10).map((c => c.title["$t"])).join("، ");
                return global.safeSend(api, `❌ لا يوجد فصل يطابق "${chapterQuery}" في "${mangaTitle}".\n` + `📚 أمثلة على العناوين: ${sample} ...`, threadID, null, messageID);
            }
            if (ambiguous && candidates.length > 1) {
                const opts = candidates.map((c => `• ${c.title["$t"]}`)).join("\n");
                return global.safeSend(api, `⚠️ "${chapterQuery}" يطابق أكثر من فصل في "${mangaTitle}":\n${opts}\n\n` + `حدّد بالضبط (مثال: annual 1 لتمييز العدد السنوي).`, threadID, null, messageID);
            }
            ...
            if (images.length === 0) {
                return global.safeSend(api, `❌ لم يتم العثور على صور في هذا الفصل.\n` + `• العنوان: ${chapterTitle}\n` + `• الرابط: ${chapterUrl}`, threadID, null, messageID);
            }
            if (images.length > MAX_IMAGES) {
                const totalFound = images.length;
                images = images.slice(0, MAX_IMAGES);
                await new Promise((resolve => {
                    global.safeSend(api, `⚠️ هذا الفصل يحتوي ${totalFound} صفحة، وسيتم إرسال أول ${MAX_IMAGES} صفحة فقط (حد أمان).`, threadID, resolve, messageID);
                }));
            }
            ...
            if (!validFiles.length) {
                return global.safeSend(api, `❌ فشل تحميل صور هذا الفصل. حاول مرة أخرى لاحقاً.`, threadID, null, messageID);
            }
            let allSent = true;
            ...
            if (!allSent || failedCount > 0) {
                await global.safeSend(api, `⚠️ ${mangaTitle} • ${chapterTitle}\n` + `تعذّر تحميل/إرسال بعض الصور (${failedCount + (allSent ? 0 : validFiles.length)} من أصل ${images.length} تقريباً).`, threadID, null, null);
            }
        } catch (error) {
            console.error("[comic] خطأ:", error.message);
            let errorMsg = `❌ حدث خطأ:\n`;
            if (error.code === "ECONNABORTED") errorMsg += `انتهت المهلة. حاول مرة أخرى.`; else if (error.response) errorMsg += `الخادم رد بـ ${error.response.status}`; else errorMsg += error.message;
