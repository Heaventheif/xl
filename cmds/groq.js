...
function detectAttachment(event) {
    ...
    for (const att of sources) {
        ...
        if (type === "file" || type === "document") {
            ...
            if ([ "mp3", "m4a", "ogg", "wav", "flac", "aac" ].includes(ext)) return {
                kind: "audio",
                url: url
            };
...
function sanitizeName(name) {
    ...
    const clean = String(name).replace(/[\u0000-\u001F\u007F]/g, "").replace(/[[\]{}<>`]/g, "").replace(/\s+/g, " ").trim().slice(0, 40);
    return clean || "مستخدم";
}
...
async function callHF(messages, attachment, prompt, wantsVoice) {
    ...
    const {data: data} = await http.post(`${getHfBase()}/groq`, body, {
        timeout: 6e4,
        headers: {
...
async function sendVoiceReply(api, threadID, messageID, audioBase64, format = "wav") {
    const tmpPath = path.join(os.tmpdir(), `groq-tts-${Date.now()}-${Math.random().toString(36).slice(2)}.${format}`);
    try {
        fs.writeFileSync(tmpPath, Buffer.from(audioBase64, "base64"));
        const info = await new Promise(((resolve, reject) => api.sendMessage({
...
async function handle(api, event, prompt, registerReply) {
    ...
    if (repliedBody && ![ "clear", "مسح", "reset" ].includes(prompt.trim().toLowerCase())) {
        prompt = prompt.trim() ? `[رد على]: "${repliedBody}"\n${prompt.trim()}` : `[رد على]: "${repliedBody}"`;
    }
    ...
    if (!prompt.trim() && !attachment) {
        return global.safeSend(api, "❓ اكتب سؤالك أو أرسل صورة/صوت/فيديو، وسأرد عليك بصوت 🔊!\n" + "مثال: .groq كم ناتج 1+8؟\n" + ".groq مسح — لمسح ذاكرة المجموعة", threadID, null, messageID);
    }
    ...
    try {
        const sent = await new Promise(((resolve, reject) => global.safeSend(api, attachment ? `⏳ جاري تحليل ${attachment.kind === "image" ? "الصورة 🖼️" : attachment.kind === "audio" ? "الصوت 🎵" : "الفيديو 🎬"}...` : "⏳ جاري توليد الرد الصوتي 🔊...", threadID, ((err, info) => err ? reject(err) : resolve(info)), messageID)));
    ...
    try {
        ...
        const msg = e.message?.includes("HF_SPACE_URL") ? "❌ HF_SPACE_URL غير مضبوط في متغيرات البيئة." : "❌ الخادم غير متاح حالياً، حاول لاحقاً.";
    ...
    if (result.audio) {
        if (statusMsgId) {
            try {
                await api.unsendMessage(statusMsgId, threadID);
            } catch (_) {}
        }
        ...
        await updateStatus((result.tts_error ? "⚠️ تعذّر توليد الصوت، إليك الرد نصياً:\n\n" : "") + reply);
    }
    if (followUpMsgId && registerReply) {
        registerReply(followUpMsgId, {}, (async ({api: api, event: event}) => {
            await handle(api, event, event.body?.trim() || "", registerReply);
        }), senderID);
    }
    await saveCtx(sessionKey, [ ...ctx, {
        role: "user",
        content: userContent
    }, {
        role: "assistant",
...
export default {
    config: {
        ...
        usage: [ "{pn}Ai4 <سؤالك> — يرد البوت برسالة صوتية 🔊", "{pn}Ai4 + صورة/صوت/فيديو مرفق — تحليل الوسائط والرد بصوت", "{pn}Ai4 مسح — مسح ذاكرة المحادثة الجماعية" ]
    },
    onStart: async ({api: api, event: event, args: args, message: message}) => {
        const prompt = args.join(" ").trim() || "";
        await handle(api, event, prompt, message?.registerReply);
    },
    onReply: async ({api: api, event: event, message: message}) => {
