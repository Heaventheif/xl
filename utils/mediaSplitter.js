...
const TARGET_PART_BYTES = Math.floor(MAX_PART_BYTES * .9);
...
function runFfmpegSegment(filePath, ext, segmentSeconds, outDir) {
    ...
    return new Promise(((resolve, reject) => {
        execFile("ffmpeg", [ "-y", "-i", filePath, "-c", "copy", "-map", "0", "-f", "segment", "-segment_time", String(Math.max(1, Math.floor(segmentSeconds))), "-reset_timestamps", "1", pattern ], {
            timeout: 10 * 60 * 1e3,
...
async function segmentOnce(filePath, ext, size, workDir) {
    ...
    const files = (await fs.readdir(workDir)).filter((f => f.startsWith("part_"))).sort().map((f => path.join(workDir, f)));
