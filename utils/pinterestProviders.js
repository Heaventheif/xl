...
async function searchViaHfBridge(query, limit) {
    ...
    const {data: data} = await http.post(`${HF_SPACE_URL}/pinterest`, {
        query: query,
        limit: limit,
        quality: "original",
        fallback_ferdev: true
    }, {
        timeout: 3e4,
        headers: {
...
async function searchViaFerdev(query, limit) {
    ...
    const response = await http.get("https://api.ferdev.my.id/search/pinterest", {
        ...
        timeout: 3e4,
        headers: {
            "User-Agent": "SunkenBot/2.0"
        }
    });
    const rawResults = response.data?.result;
    if (!rawResults?.length) throw new Error("لا نتائج");
    const urls = rawResults.slice(0, limit).map((item => item.url || item.image || (typeof item === "string" ? item : null))).filter(Boolean);
    ...
    async function worker() {
        while (idx < urls.length) {
            ...
            try {
                ...
                if (!imgBuffer || imgBuffer.length < 1e3) continue;
...
export const $plugin = {
    ...
    meta: {
        category: "utils",
        path: "utils/pinterestProviders.js"
    },
    setup(_ctx) {}
};