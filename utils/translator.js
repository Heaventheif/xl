...
(async () => {
    try {
        _googleXTranslate = (await import("google-translate-api-x")).translate;
    } catch (_) {}
    try {
        _bingTranslate = (await import("bing-translate-api")).translate;
    } catch (_) {}
})();
...
function isProviderErrorText(text) {
    ...
    return t.includes("query length limit exceeded") || t.includes("max allowed query") || text.includes("تجاوز حد طول الاستعلام") || text.includes("الحد الأقصى المسموح به للاستعلام");
}
...
const providers = [ {
    ...
    run: async (text, targetLang) => {
        ...
        const res = await withTimeout(googleXTranslate(text, {
            to: targetLang
        }), TIMEOUT_MS, "Google-X");
    ...
    for (const provider of providers) {
        ...
        try {
            ...
            if (isRateLimited(e)) {
                ...
                console.warn(`[TRANSLATOR] حظر مؤقت لـ ${provider.name} (${provider.cooldownMs / 6e4} دقيقة) — تحويل للمحرك التالي`);
            } else {
...
async function translateToArabic(text) {
    if (!text?.trim()) return text;
    if (/[\u0600-\u06FF]/.test(text) && text.match(/[\u0600-\u06FF]/g).length > text.length * .3) {
        return text;
    }
...
async function translateToEnglish(text) {
    ...
    if (!nonAsciiLetters || nonAsciiLetters.length < text.length * .15) {
        return text;
    }
